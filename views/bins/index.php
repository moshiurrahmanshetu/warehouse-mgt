...
<div class="d-flex">
    ...
    <main class="wms-main p-4 flex-grow-1">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h2>Bins</h2>
            <?php if(hasPermission('bins.manage')): ?>
            <a href="bins.php?action=create" class="btn btn-primary">Add New</a>
            <?php endif; ?>
        </div>
        <?php renderFlash(); ?>
        <div class="card">
            <div class="card-body">
                <table class="table table-bordered table-striped" id="dataTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= e($item['id']) ?></td>
                            <td><?= e($item['bin_code']) ?></td>
                            <td><?= e($item['bin_name']) ?></td>
                            <td><span class="badge bg-<?= $item['status'] === 'active' ? 'success' : 'danger' ?>"><?= ucfirst(e($item['status'])) ?></span></td>
                            <td>
                                <?php if(hasPermission('bins.manage')): ?>
                                <a href="bins.php?action=edit&id=<?= $item['id'] ?>" class="btn btn-sm btn-warning">Edit</a>
                                <form action="bins.php?action=delete" method="POST" class="d-inline" onsubmit="return confirm('Delete this Bin?');">
                                    <?= csrfField() ?>
